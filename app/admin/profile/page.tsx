...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me') // We need this endpoint or parse from elsewhere. Let's create it or use a trick. 
      // Actually we didn't create /api/auth/me yet. 
      // We can fetch from client side decoding but that's insecure/messy. 
      // Let's create `app/api/auth/me/route.ts` quickly or just use `api/users` filter? No.
      // Better to create `app/api/auth/me/route.ts` which returns current session user.
      .then(res => {
