...
export default function SettingsPage() {
  ...
  const fetchCurrentUser = async () => {
    try {
        ...
        if (res.ok) {
            ...
            setCurrentUser(data);
            // Default to 'team' if Super Admin for convenience, or remain 'profile'
        }
    } catch (e) {
        console.error(e);
    }
  ...
  const runCleanup = async () => {
      ...
      try {
          const res = await fetch('/api/admin/lifecycle/cleanup', { method: 'POST' });
          if(res.ok) {
  ...
  useEffect(() => {
    fetchCurrentUser().then(() => {
        // Only fetch all users if we suspect we might have access (we'll see 403 otherwise but that's fine)
        fetchUsers();
        fetchClients();
    });
  }, []);
  ...
  const handleSavePermissions = async () => {
    ...
    try {
        const res = await fetch(`/api/users/${selectedUser.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ permissions: permChanges, role: roleChange }),
        });
        if (res.ok) {
                    ...
                    {storageStats.cloudinary && !storageStats.cloudinary.error ? (
                        <div className="flex-1 md:ml-8 max-w-2xl">
                             <div className="flex justify-between text-xs text-text-sub-600 mb-1">
                                 <span>Plan Usage: {storageStats.cloudinary.credits?.percent.toFixed(2)}%</span>
                                 <span>{storageStats.cloudinary.credits?.limit} Credits Limit</span>
                             </div>
